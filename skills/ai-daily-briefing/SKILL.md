...
| 脚本 | 功能 |
|------|------|
| `fetch_all.py` | 主抓取脚本，覆盖37个源 |
| `fetch_jiqizhixin.py` | 机器之心专用（已整合） |
| `fetch_ailab.py` | 新智元专用（已整合） |
...
### 高优先级 (剩余政府网站)
- 武汉市发改委、数据局、人社局（页面结构待分析）
- 工信部: HTTP 403，需研究绕过方案

### 中优先级 (特殊限制)
- 湖北省政府: HTTP 412，需处理预检限制
- OpenAI News: HTTP 403，需改用网页抓取

### 已实现 ✅ (2026-03-22)
- ✅ 中央部委: 国务院、发改委、网信办
- ✅ 地方政府: 上海、广东、深圳、武汉市政府
- ✅ 武汉部门: 经信局、科技局、商务局、卫健委
