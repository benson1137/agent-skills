...
"""
机器之心新闻抓取脚本
先尝试requests，失败则使用playwright浏览器
"""
...
def fetch_with_playwright():
    """使用playwright浏览器抓取"""
    print("[机器之心] 使用 Playwright 浏览器抓取...")
    ...
    try:
        ...
        for item in items[:10]:
            ...
            if title_elem:
                ...
                # 提取链接 - 从外层div的点击事件或找到包裹的a标签
                link_elem = item.select_one('a') or item.find_parent('a')
                href = ''
                ...
                if not href and title:
                    # 从文章详情页搜索链接（简化处理）
                    href = f"https://www.jiqizhixin.com/articles/search?query={title[:20]}"
